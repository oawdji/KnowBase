...
| # | 问题 | 说明 |
|---|---|---|
| 8 | **多知识库只检索第一个** | `chat_service.py` 中的 `vector_stores[0]`：一个对话挂多个知识库时只有第 1 个生效 |
| 9 | 无重排、无混合检索、无相似度阈值、k 固定为 4；按固定长度切块 | 代码中不存在相关逻辑；更相关的块排在第 5 名也无法纠正 |

#### 功能性缺陷

| # | 问题 | 说明 |
|---|---|---|
| 10 | 同名文件重复上传 | 触发唯一约束 `uq_kb_file_name` → 事务回滚异常 → 任务**永久停在 processing** |
| 11 | 增量更新是死代码 | `process_document()` / `ChunkRecord` 从未被调用；实际只追加，改过的文档会叠加旧块 |
| 12 | 没有按文档删除的接口 | 只能整库删除 |
| 13 | MySQL 与 Chroma 双写不保证一致 | Chroma 写入失败不回滚数据库 |
| 14 | 孤儿任务行 | 删库后 `processing_tasks` 残留记录（`knowledge_base_id` 为 NULL，实测 2 行） |
| 15 | 无任务队列 | 进程重启会丢失进行中的入库任务，任务状态卡住 |
| 16 | 开放接口无法通过网关访问 | nginx 缺少 `/openapi/` 路由 → 页面里写的调用方式必然 404 |

#### 安全与健壮性

| # | 问题 | 说明 |
|---|---|---|
| 17 | **API 密钥明文存储，且明文写进日志** | `api_keys.key` 是明文列；`api_keys.py` 的创建 / 修改 / 删除三处都用 `logger.info` 打印**完整密钥**，`docker compose logs` 即可拿到可用凭据。对比：「模型设置」里的 Key 是用 Fernet 加密存的 |
| 18 | **`SECRET_KEY` 有可用的默认值** | 默认 `your-secret-key-here`，直接用于签发 JWT。若部署时忘记设置，任何人都能伪造登录令牌 |
| 19 | **`/cleanup` 接口没有用户隔离** | 任何登录用户调用都会清理**全体用户**超过 24 小时的临时上传记录与 MinIO 对象 |
| 20 | 文档入库会阻塞整个后端 | `process_document_background` 声明为 `async` 但内部**没有任何 `await`**（下载、解析、向量化全是同步阻塞调用），却用 `asyncio.create_task` 丢进事件循环；单 worker 下解析大文档期间，聊天等所有请求一起排队。另外 `create_task` 的返回值未保存，任务有被 GC 提前回收的风险 |
| 21 | 上传没有类型与大小校验 | `await file.read()` 把整个文件读进内存，扩展名、体积均不校验（不在白名单内的扩展名会在入库阶段才失败） |

#### 工程卫生

| # | 问题 | 说明 |
|---|---|---|
| 22 | 后端测试跑不起来 | 仓库有 `backend/tests/test_minimax_*.py`，但 `requirements.txt` 里没有 `pytest`，CI 里也没跑它们 |
| 23 | CI 判定偏弱 | `.github/workflows/test.yml` 用 `curl -v` 且没有 `--fail`，只 `sleep 30` 硬等 → 服务没起来也可能判为通过 |
| 24 | Markdown 排版实际未生效 | 前端用了 `prose` 类，但 `@tailwindcss/typography` **没有安装**，`tailwind.config` 的 plugins 里也没有 → AI 回答的标题、列表、表格没有层级与间距 |
| 25 | `uploads/` 不在 `.gitignore` | 目前是空目录所以侥幸未被提交，一旦有文件写入就会被 git 跟踪 |
| 26 | 生产环境不支持「本地模型 + 离线」这套配置 | 见[第 6 节](#6-配置说明)的 ⚠️ 提示：生产 compose 没有 `./models` 挂载、没有 `hf_cache` 卷、也没有 `HF_HUB_OFFLINE` |
| 27 | LangChain 的全局调试开关是打开的 | `chat_service.py` 顶部有 `set_verbose(True)` / `set_debug(True)`：日志噪音极大，且会拖慢流式输出 |
...
4. **入库任务移出事件循环**：`process_document_background` 放进线程池 / 独立 worker，避免大文档卡住整个后端
5. **上传加校验**：扩展名白名单 + 体积上限

### 第三批：提升检索质量

6. 引入混合检索（BM25 + 向量 + RRF 融合）与重排模型（`bge-reranker-v2-m3`）；修多知识库联合检索（目前只用第一个）
7. 建 20–50 条黄金问答集，用 Recall@k / RAGAS 量化效果
8. 切块策略升级（按标题 / 语义切分），给检索加相似度阈值

### 第四批：工程卫生

9. 关闭 `chat_service.py` 中的 `set_verbose(True)` / `set_debug(True)`（日志噪音极大且拖慢流式输出）
10. 修同名重传与死代码（增量更新）、增加按文档删除、清理孤儿任务行
11. 补测试与 CI：`requirements.txt` 加 `pytest` 并让 CI 真跑后端测试、`curl` 加 `--fail`、安装 `@tailwindcss/typography` 让 `prose` 生效
12. 开放接口页面：或补齐 nginx 路由，或下架
